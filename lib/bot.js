...
var Bot = function(options) {
  ...
  this.bot_token = options.bot.config.token;
  ...
  this.setUpBot = function(onSuccess) {
  ...
  this.setUpChannels = function(channels) {
    channels.forEach(function(channel) {
      if (!channel.is_archived) {
        this.allChannels.push(channel);
        if (channel.members && channel.members.indexOf(this.botId) > -1 && this.memberChannels.indexOf(channel.id) < 0) {
  ...
  this.setUpUsers = function(users) {
    this.allUsers = [];
    for (var u in users) {
      var user = users[u];
      if (!user.deleted) {
  ...
  this.startInterval = function() {
    ...
    this.checkInterval = setInterval( function () {
      ...
      _thi.getChannelActivity(bot, 'recent', function (channel, isLast) {
        if (channel) {
  ...
  this.getChannelHistory = function(channel, type, callback) {
  ...
  this.getChannelActivity = function(type, callback) {
  ...
  this.channelIsDead = function(channel) {
    return this.deadChannels.find(function(ch) { return ch.id === channel.id });
  };

  this.channelIsActive = function(messages, minimum) {
    var users = [];
    var messageCount = 0;
  ...
  this.sendReply = function(message, channelList, type) {
    this.reply(message, this.formatMessage(channelList, type));
  };

  this.formatMessage = function(channelList, type) {
  ...
  this.formatBotText = function(channelList, type) {
    var text = 'The ';
  ...
  this.formatChannelName = function(channelName) {
    if (this.allChannels && this.allChannels.length) {
