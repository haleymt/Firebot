...
var Firebot = {
  ...
  configureEventEndpoints: function() {
    ...
    controller.webserver.post('/slack/receive', function(req, res) {
      ...
      if (challenge) {
        ...
        this.handleReceiveMessage(team_id, event);
      }
    }.bind(this));
  },
  ...
  handleReceiveMessage: function(teamId, message) {
    const bot = this.bots[teamId];
    if (!bot.channelActivity[message.channel]) {
      bot.channelActivity[message.channel] = { messageCount: 1, users: [message.user]};
    } else {
      let { messageCount, users } = bot.channelActivity[message.channel];
      messageCount += 1;
      if (users.indexOf(message.user) < 0) {
        users.push(message.user);
      }

      bot.channelActivity[message.channel] = { messageCount, users };
    }

    console.log(bot.channelActivity);
  },
